...
resource "google_secret_manager_regional_secret" "this" {
  ...
  dynamic "customer_managed_encryption" {
    for_each = var.customer_managed_encryption == null ? [] : (can(tolist(var.customer_managed_encryption)) ? tolist(var.customer_managed_encryption) : [var.customer_managed_encryption])
    content {}
  }
  dynamic "rotation" {
