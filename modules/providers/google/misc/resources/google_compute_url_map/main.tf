...
resource "google_compute_url_map" "this" {
  ...
  dynamic "default_custom_error_response_policy" {
    for_each = var.default_custom_error_response_policy == null ? [] : (can(tolist(var.default_custom_error_response_policy)) ? tolist(var.default_custom_error_response_policy) : [var.default_custom_error_response_policy])
    content {}
  }
